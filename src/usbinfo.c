...
static void dump_junk(const unsigned char *buf, const char *indent, unsigned int len)
{
	...
	if (buf[0] <= len)
		return;
	win_printf("%sjunk at descriptor end:", indent);
	for (i = len; i < buf[0]; i++)
		win_printf(" %02x", buf[i]);
	win_printf("\n");
}
...
static void dump_config(libusb_device_handle *dev, struct libusb_config_descriptor *config)
{
	...
	if (!(config->bmAttributes & 0x80))
		info_printf("    Missing must-be-set bit!\n");
	if (config->bmAttributes & 0x40)
		info_printf("    Self Powered\n");
	else
		info_printf("    Bus Powered\n");
	if (config->bmAttributes & 0x20)
		info_printf("    Remote Wakeup\n");
	if (config->bmAttributes & 0x10)
		info_printf("    Battery Powered\n");
	info_printf("    MaxPower：     %5umA\n", config->MaxPower * 2);
	...
	info_printf("Interface Numbers： %d\n", config->bNumInterfaces);
	for (i = 0 ; i < config->bNumInterfaces ; i++) {
		info_printf("        Interface %d\n", i);
...
static void dump_interface(libusb_device_handle *dev, const struct libusb_interface *interface)
{
	...
	info_printf("        Setting Numbers: %d\n", interface->num_altsetting);
	for (i = 0; i < interface->num_altsetting; i++) {
		info_printf("                Setting%d\n", i);
...
static void dump_unit(unsigned int data, unsigned int len)
{
	...
	if (sys > 4) {
		if (sys == 0xf)
			win_printf("System: Vendor defined, Unit: (unknown)\n");
		else
			win_printf("System: Reserved, Unit: (unknown)\n");
		return;
	} else {
		win_printf("System: %s, Unit: ", systems[sys]);
	}
	for (i = 1 ; i < len * 2 ; i++) {
...
static void dump_audiocontrol_interface(libusb_device_handle *dev, const unsigned char *buf, int protocol)
{
	...
	if (buf[1] != USB_DT_CS_INTERFACE)
		win_printf("      Warning: Invalid descriptor\n");
	else if (buf[0] < 3)
		win_printf("      Warning: Descriptor too short\n");
	...
	case 0x01:  /* HEADER */
		...
		case USB_AUDIO_CLASS_1:
			if (buf[0] < 8+buf[7])
				win_printf("      Warning: Descriptor too short\n");
			win_printf("        bcdADC              %2x.%02x\n"
			       "        wTotalLength        %5u\n"
			       "        bInCollection       %5u\n",
			       buf[4], buf[3], buf[5] | (buf[6] << 8), buf[7]);
			for (i = 0; i < buf[7]; i++)
				win_printf("        baInterfaceNr(%2u)   %5u\n", i, buf[8+i]);
	...
	case 0x04:  /* MIXER_UNIT */
		...
		case USB_AUDIO_CLASS_1:
			...
			k = buf[j+5];
			if (j == 0 || k == 0) {
				win_printf("      Warning: mixer with %5u input and %5u output channels.\n", j, k);
				N = 0;
			} else {
				N = 1+(j*k-1)/8;
			}
			...
			win_printf("        bUnitID             %5u\n"
			       "        bNrInPins           %5u\n",
			       buf[3], buf[4]);
			for (i = 0; i < j; i++)
				win_printf("        baSourceID(%2u)      %5u\n", i, buf[5+i]);
			win_printf("        bNrChannels         %5u\n"
			       "        wChannelConfig     0x%04x\n",
			       buf[5+j], chcfg);
			for (i = 0; i < 12; i++)
				if ((chcfg >> i) & 1)
			...
			win_printf("        iChannelNames       %5u %s\n",
			       buf[8+j], chnames);
			for (i = 0; i < N; i++)
				win_printf("        bmControls         0x%02x\n", buf[9+j+i]);
			win_printf("        iMixer              %5u %s\n", buf[9+j+N], term);
	...
	case 0x05:  /* SELECTOR_UNIT */
		...
		case USB_AUDIO_CLASS_1:
			...
			win_printf("        bUnitID             %5u\n"
			       ...
			       buf[3], buf[4]);
			for (i = 0; i < buf[4]; i++)
				win_printf("        baSource(%2u)        %5u\n", i, buf[5+i]);
			win_printf("        iSelector           %5u %s\n",
			       buf[5+buf[4]], term);
	...
	case 0x06:  /* FEATURE_UNIT */
		...
		case USB_AUDIO_CLASS_1:
			...
			for (i = 0; i < k; i++) {
				...
				for (j = 0; j < buf[5]; j++)
					win_printf("        bmaControls(%2u)      0x%02x\n", i, buf[6+buf[5]*i+j]);

				dump_audio_bmcontrols("          ", chcfg, uac_fu_bmcontrols, protocol);
			}
			win_printf("        iFeature            %5u %s\n", buf[6+buf[5]*k], term);
		...
		case USB_AUDIO_CLASS_2:
			...
			for (i = 0; i < k; i++) {
				chcfg = buf[5+(4*i)] |
					...
					buf[7+(4*i)] << 16 |
					buf[8+(4*i)] << 24;
				win_printf("        bmaControls(%2u)      0x%08x\n", i, chcfg);
				dump_audio_bmcontrols("          ", chcfg, uac_fu_bmcontrols, protocol);
			}
	...
	case 0x07:  /* PROCESSING_UNIT */
		...
		case USB_AUDIO_CLASS_1:
			...
			for (i = 0; i < k; i++)
				win_printf("        bmControls(%2u)       0x%02x\n", i, buf[12+j+i]);
			if (buf[12+j] & 1)
				win_printf("          Enable Processing\n");
			win_printf("        iProcessing         %5u %s\n"
	...
	case 0x0a:  /* CLOCK_SOURCE */
		win_printf ("(CLOCK_SOURCE)\n");
		if (protocol != USB_AUDIO_CLASS_2)
			win_printf("      Warning: CLOCK_SOURCE descriptors are illegal for UAC1\n");

		if (buf[0] < 8)
			win_printf("      Warning: Descriptor too short\n");

		win_printf("        bClockID            %5u\n"
		...
		win_printf("        bAssocTerminal      %5u\n", buf[6]);
		win_printf("        iClockSource        %5u %s\n", buf[7], term);
		dump_junk(buf, "        ", 8);
		break;

	case 0x0b:  /* CLOCK_SELECTOR */
		win_printf("(CLOCK_SELECTOR)\n");
		if (protocol != USB_AUDIO_CLASS_2)
			win_printf("      Warning: CLOCK_SELECTOR descriptors are illegal for UAC1\n");
		...
		win_printf("        bUnitID             %5u\n"
		       ...
		       buf[3], buf[4]);
		for (i = 0; i < buf[4]; i++)
			win_printf("        baCSourceID(%2u)     %5u\n", i, buf[5+i]);
		win_printf("        bmControls           0x%02x\n", buf[5+buf[4]]);
		dump_audio_bmcontrols("          ", buf[5+buf[4]], uac2_clock_selector_bmcontrols, protocol);

		win_printf("        iClockSelector      %5u %s\n",
		       buf[6+buf[4]], term);
		dump_junk(buf, "        ", 7+buf[4]);
		break;

	case 0x0c:  /* CLOCK_MULTIPLIER */
		win_printf("(CLOCK_MULTIPLIER)\n");
		if (protocol != USB_AUDIO_CLASS_2)
			win_printf("      Warning: CLOCK_MULTIPLIER descriptors are illegal for UAC1\n");
		...
		win_printf("        iClockMultiplier    %5u %s\n", buf[6], term);
		dump_junk(buf, "        ", 7);
		break;

	case 0x0d:  /* SAMPLE_RATE_CONVERTER_UNIT */
		win_printf("(SAMPLE_RATE_CONVERTER_UNIT)\n");
		if (protocol != USB_AUDIO_CLASS_2)
			win_printf("      Warning: SAMPLE_RATE_CONVERTER_UNIT descriptors are illegal for UAC1\n");
...
static void dump_audiostreaming_interface(libusb_device_handle *dev, const unsigned char *buf, int protocol)
{
	...
	case 0x01: /* AS_GENERAL */
		...
		case USB_AUDIO_CLASS_2:
			...
			win_printf("        bFormatType         %5u\n", buf[5]);
			fmttag = buf[6] | (buf[7] << 8) | (buf[8] << 16) | (buf[9] << 24);
			win_printf("        bmFormats           %5u\n", fmttag);
			for (i=0; i<5; i++)
				if ((fmttag >> i) & 1)
					win_printf("          %s\n", fmtItag[i+1]);
	...
	case 0x02: /* FORMAT_TYPE */
		win_printf("(FORMAT_TYPE)\n");
		switch (protocol) {
		case USB_AUDIO_CLASS_1:
			if (buf[0] < 8)
				win_printf("      Warning: Descriptor too short\n");
			win_printf("        bFormatType         %5u ", buf[3]);
			switch (buf[3]) {
			case 0x01: /* FORMAT_TYPE_I */
				win_printf("(FORMAT_TYPE_I)\n");
	...
	case 0x03: /* FORMAT_SPECIFIC */
		...
		if (fmttag <= 5)
			...
			fmtptr = fmtIIItag[fmttag & 0xfff];
		win_printf("        wFormatTag          %5u %s\n", fmttag, fmtptr);
		switch (fmttag) {
		case 0x1001: /* MPEG */
			if (buf[0] < 8)
				win_printf("      Warning: Descriptor too short\n");
			win_printf("        bmMPEGCapabilities 0x%04x\n",
			       buf[5] | (buf[6] << 8));
			if (buf[5] & 0x01)
				win_printf("          Layer I\n");
			if (buf[5] & 0x02)
				win_printf("          Layer II\n");
			if (buf[5] & 0x04)
				win_printf("          Layer III\n");
			if (buf[5] & 0x08)
				win_printf("          MPEG-1 only\n");
			if (buf[5] & 0x10)
				win_printf("          MPEG-1 dual-channel\n");
			if (buf[5] & 0x20)
				win_printf("          MPEG-2 second stereo\n");
			if (buf[5] & 0x40)
				win_printf("          MPEG-2 7.1 channel augmentation\n");
			if (buf[5] & 0x80)
				win_printf("          Adaptive multi-channel prediction\n");
			win_printf("          MPEG-2 multilingual support: ");
			switch (buf[6] & 3) {
			case 0:
				win_printf("Not supported\n");
				break;

			case 1:
				win_printf("Supported at Fs\n");
				break;
			...
			default:
				win_printf("Supported at Fs and 1/2Fs\n");
				break;
			}
			win_printf("        bmMPEGFeatures       0x%02x\n", buf[7]);
			win_printf("          Internal Dynamic Range Control: ");
			switch ((buf[7] << 4) & 3) {
			case 0:
				win_printf("not supported\n");
				break;
			...
			case 1:
				win_printf("supported but not scalable\n");
				break;

			case 2:
				win_printf("scalable, common boost and cut scaling value\n");
				break;

			default:
				win_printf("scalable, separate boost and cut scaling value\n");
				break;
			}
			dump_junk(buf, "        ", 8);
			break;
		...
		case 0x1002: /* AC-3 */
			if (buf[0] < 10)
				win_printf("      Warning: Descriptor too short\n");
			win_printf("        bmBSID         0x%08x\n"
			       "        bmAC3Features        0x%02x\n",
			       buf[5] | (buf[6] << 8) | (buf[7] << 16) | (buf[8] << 24), buf[9]);
			if (buf[9] & 0x01)
				win_printf("          RF mode\n");
			if (buf[9] & 0x02)
				win_printf("          Line mode\n");
			if (buf[9] & 0x04)
				win_printf("          Custom0 mode\n");
			if (buf[9] & 0x08)
				win_printf("          Custom1 mode\n");
			win_printf("          Internal Dynamic Range Control: ");
			switch ((buf[9] >> 4) & 3) {
			case 0:
				win_printf("not supported\n");
	...
	default:
		win_printf("        Invalid desc subtype:");
		dump_bytes(buf+3, buf[0]-3);
		break;
	}
}
...
static void dump_audiostreaming_endpoint(const unsigned char *buf, int protocol)
{
	...
	case USB_AUDIO_CLASS_1:
		if (buf[3] & 1)
			win_printf("            Sampling Frequency\n");
		if (buf[3] & 2)
			win_printf("            Pitch\n");
		if (buf[3] & 128)
			win_printf("            MaxPacketsOnly\n");
		lckdelidx = buf[4];
		if (lckdelidx > 3)
			lckdelidx = 3;
		win_printf("          bLockDelayUnits     %5u %s\n"
		       "          wLockDelay          %5u %s\n",
...
static void dump_midistreaming_interface(libusb_device_handle *dev, const unsigned char *buf)
{
	...
	case 0x02:
		win_printf("(MIDI_IN_JACK)\n");
		if (buf[0] < 6)
			win_printf("      Warning: Descriptor too short\n");
		get_string(dev, jackstr, sizeof(jackstr), buf[5]);
		win_printf("        bJackType           %5u %s\n"
		       "        bJackID             %5u\n"
		       "        iJack               %5u %s\n",
	...
	case 0x04:
		...
		win_printf("        bmElementCaps  0x%08lx\n", caps);
		if (caps & 0x01)
			win_printf("          Undefined\n");
		if (caps & 0x02)
			win_printf("          MIDI Clock\n");
		if (caps & 0x04)
			win_printf("          MTC (MIDI Time Code)\n");
		if (caps & 0x08)
			win_printf("          MMC (MIDI Machine Control)\n");
		if (caps & 0x10)
			win_printf("          GM1 (General MIDI v.1)\n");
		if (caps & 0x20)
			win_printf("          GM2 (General MIDI v.2)\n");
		if (caps & 0x40)
			win_printf("          GS MIDI Extension\n");
		if (caps & 0x80)
			win_printf("          XG MIDI Extension\n");
		if (caps & 0x100)
			win_printf("          EFX\n");
		if (caps & 0x200)
			win_printf("          MIDI Patch Bay\n");
		if (caps & 0x400)
			win_printf("          DLS1 (Downloadable Sounds Level 1)\n");
		if (caps & 0x800)
			win_printf("          DLS2 (Downloadable Sounds Level 2)\n");
		j = 9+2*buf[4]+capssize;
		get_string(dev, jackstr, sizeof(jackstr), buf[j]);
		win_printf("        iElement            %5u %s\n", buf[j], jackstr);
		dump_junk(buf, "        ", j+1);
		break;
...
static void dump_midistreaming_endpoint(const unsigned char *buf)
{
	...
	for (j = 0; j < buf[3]; j++)
		win_printf("          baAssocJackID(%2u)   %5u\n", j, buf[4+j]);
...
static void dump_videocontrol_interface(libusb_device_handle *dev, const unsigned char *buf)
{
	...
	case 0x02:  /* INPUT_TERMINAL */
		...
		if (termt == 0x0201) {
			n += buf[14];
			win_printf("        wObjectiveFocalLengthMin  %5u\n"
...
static void dump_videostreaming_interface(const unsigned char *buf)
{
	...
	case 0x01: /* INPUT_HEADER */
		win_printf("(INPUT_HEADER)\n");
		p = buf[3];
		n = buf[12];
		if (buf[0] < 13+p*n)
			win_printf("      Warning: Descriptor too short\n");
		win_printf("        bNumFormats                     %5u\n"
		       "        wTotalLength                    %5u\n"
		       "        bEndPointAddress                %5u\n"
		       "        bmInfo                          %5u\n"
		       "        bTerminalLink                   %5u\n"
	...
	case 0x03: /* STILL_IMAGE_FRAME */
		win_printf("(STILL_IMAGE_FRAME)\n");
		n = buf[4];
		m = buf[5+4*n];
		if (buf[0] < 6+4*n+m)
			win_printf("      Warning: Descriptor too short\n");
		win_printf("        bEndpointAddress                %5u\n"
		       "        bNumImageSizePatterns             %3u\n",
		       buf[3], n);
		for (i = 0; i < n; i++)
			win_printf("        wWidth(%2u)                      %5u\n"
			       "        wHeight(%2u)                     %5u\n",
			       i, buf[5+4*i] | (buf[6+4*i] << 8),
			       i, buf[7+4*i] | (buf[8+4*i] << 8));
		win_printf("        bNumCompressionPatterns           %3u\n", n);
		for (i = 0; i < m; i++)
			win_printf("        bCompression(%2u)                %5u\n",
			       i, buf[6+4*n+i]);
	...
	case 0x10: /* FORMAT_FRAME_BASED */
		if (buf[2] == 0x04) {
			win_printf("(FORMAT_UNCOMPRESSED)\n");
			len = 27;
		} else {
			win_printf("(FORMAT_FRAME_BASED)\n");
			len = 28;
		}
		if (buf[0] < len)
			win_printf("      Warning: Descriptor too short\n");
		flags = buf[25];
		win_printf("        bFormatIndex                    %5u\n"
		       ...
		       buf[23], buf[24], flags);
		win_printf("          Interlaced stream or variable: %s\n",
		       (flags & (1 << 0)) ? "Yes" : "No");
		win_printf("          Fields per frame: %u fields\n",
		       (flags & (1 << 1)) ? 1 : 2);
		win_printf("          Field 1 first: %s\n",
		       (flags & (1 << 2)) ? "Yes" : "No");
		win_printf("          Field pattern: ");
		switch ((flags >> 4) & 0x03) {
		case 0:
			win_printf("Field 1 only\n");
			break;
		case 1:
			win_printf("Field 2 only\n");
			break;
		case 2:
			win_printf("Regular pattern of fields 1 and 2\n");
			break;
		case 3:
			win_printf("Random pattern of fields 1 and 2\n");
			break;
		}
		win_printf("          bCopyProtect                  %5u\n", buf[26]);
		if (buf[2] == 0x10)
			win_printf("          bVariableSize                 %5u\n", buf[27]);
		dump_junk(buf, "        ", len);
		break;
	...
	case 0x11: /* FRAME_FRAME_BASED */
		if (buf[2] == 0x05) {
			win_printf("(FRAME_UNCOMPRESSED)\n");
			n = 25;
		} else if (buf[2] == 0x07) {
			win_printf("(FRAME_MJPEG)\n");
			n = 25;
		} else {
			win_printf("(FRAME_FRAME_BASED)\n");
			n = 21;
		}
		...
		if (buf[0] < len)
			win_printf("      Warning: Descriptor too short\n");
		flags = buf[4];
		win_printf("        bFrameIndex                     %5u\n"
		       "        bmCapabilities                   0x%02x\n",
		       buf[3], flags);
		win_printf("          Still image %ssupported\n",
		       (flags & (1 << 0)) ? "" : "un");
		if (flags & (1 << 1))
			win_printf("          Fixed frame-rate\n");
		win_printf("        wWidth                          %5u\n"
		       "        wHeight                         %5u\n"
		       "        dwMinBitRate                %9u\n"
		       "        dwMaxBitRate                %9u\n",
		...
		if (buf[2] == 0x11)
			win_printf("        dwDefaultFrameInterval      %9u\n"
			...
			win_printf("        dwMaxVideoFrameBufferSize   %9u\n"
			       ...
			       buf[25]);
		if (buf[n] == 0)
			win_printf("        dwMinFrameInterval          %9u\n"
			...
			for (i = 0; i < buf[n]; i++)
				win_printf("        dwFrameInterval(%2u)         %9u\n",
	...
	case 0x06: /* FORMAT_MJPEG */
		...
		win_printf("        bFormatIndex                    %5u\n"
		       ...
		       "        bFlags                          %5u\n",
		       buf[3], buf[4], flags);
		win_printf("          Fixed-size samples: %s\n",
		       (flags & (1 << 0)) ? "Yes" : "No");
		flags = buf[9];
		win_printf("        bDefaultFrameIndex              %5u\n"
		       "        bAspectRatioX                   %5u\n"
		       "        bAspectRatioY                   %5u\n"
		       "        bmInterlaceFlags                 0x%02x\n",
	...
	case 0x0a: /* FORMAT_MPEG2TS */
		...
		if (len > 7)
			win_printf("        guidStrideFormat                      %s\n",
			       get_guid(&buf[7]));
		dump_junk(buf, "        ", len);
		break;

	case 0x0d: /* COLORFORMAT */
		win_printf("(COLORFORMAT)\n");
		if (buf[0] < 6)
			win_printf("      Warning: Descriptor too short\n");
		win_printf("        bColorPrimaries                 %5u (%s)\n",
		       buf[3], (buf[3] <= 5) ? colorPrims[buf[3]] : "Unknown");
		win_printf("        bTransferCharacteristics        %5u (%s)\n",
		       buf[4], (buf[4] <= 7) ? transferChars[buf[4]] : "Unknown");
		win_printf("        bMatrixCoefficients             %5u (%s)\n",
...
static void dump_dfu_interface(const unsigned char *buf)
{
	...
	win_printf("\nDevice Firmware Upgrade Interface Descriptor:\n"
	       ...
	       buf[0], buf[1], buf[2]);
	if (buf[2] & 0xf0)
		win_printf("          (unknown attributes!)\n");
	win_printf("          Will %sDetach\n", (buf[2] & 0x08) ? "" : "Not ");
	win_printf("          Manifestation %s\n", (buf[2] & 0x04) ? "Tolerant" : "Intolerant");
	win_printf("          Upload %s\n", (buf[2] & 0x02) ? "Supported" : "Unsupported");
	win_printf("          Download %s\n", (buf[2] & 0x01) ? "Supported" : "Unsupported");
	win_printf("        wDetachTimeout                  %5u milliseconds\n"
	...
	if (buf[0] < 9)
		return;
	win_printf("        bcdDFUVersion                   %x.%02x\n",
			buf[8], buf[7]);
}
...
static void dump_hub(const char *prefix, const unsigned char *p, int tt_type)
{
	...
	win_printf("%sHub Descriptor:\n", prefix);
	win_printf("%s  bLength             %3u\n", prefix, p[0]);
	win_printf("%s  bDescriptorType     %3u\n", prefix, p[1]);
	win_printf("%s  nNbrPorts           %3u\n", prefix, p[2]);
	win_printf("%s  wHubCharacteristic 0x%04x\n", prefix, wHubChar);
	switch (wHubChar & 0x03) {
	case 0:
		win_printf("%s    Ganged power switching\n", prefix);
		break;
	case 1:
		win_printf("%s    Per-port power switching\n", prefix);
		break;
	default:
		win_printf("%s    No power switching (usb 1.0)\n", prefix);
		break;
	}
	if (wHubChar & 0x04)
		win_printf("%s    Compound device\n", prefix);
	switch ((wHubChar >> 3) & 0x03) {
	case 0:
		win_printf("%s    Ganged overcurrent protection\n", prefix);
		break;
	case 1:
		win_printf("%s    Per-port overcurrent protection\n", prefix);
		break;
	default:
		win_printf("%s    No overcurrent protection\n", prefix);
		break;
	}
	/* USB 3.0 hubs don't have TTs. */
	if (tt_type >= 1 && tt_type < 3) {
		l = (wHubChar >> 5) & 0x03;
		win_printf("%s    TT think time %d FS bits\n", prefix, (l + 1) * 8);
	}
	/* USB 3.0 hubs don't have port indicators.  Sad face. */
	if (tt_type != 3 && wHubChar & (1<<7))
		win_printf("%s    Port indicators\n", prefix);
	win_printf("%s  bPwrOn2PwrGood      %3u * 2 milli seconds\n", prefix, p[5]);

	/* USB 3.0 hubs report current in units of aCurrentUnit, or 4 mA */
	if (tt_type == 3)
		win_printf("%s  bHubContrCurrent   %4u milli Ampere\n",
				prefix, p[6]*4);
	else
		win_printf("%s  bHubContrCurrent    %3u milli Ampere\n",
				prefix, p[6]);

	if (tt_type == 3) {
		win_printf("%s  bHubDecLat          0.%1u micro seconds\n",
				prefix, p[7]);
		win_printf("%s  wHubDelay          %4u nano seconds\n",
				prefix, (p[8] << 4) +(p[7]));
		offset = 10;
	} else {
		offset = 7;
	...
	win_printf("%s  DeviceRemovable   ", prefix);
	for (i = 0; i < l; i++)
		win_printf(" 0x%02x", p[offset+i]);

	if (tt_type != 3) {
		win_printf("\n%s  PortPwrCtrlMask   ", prefix);
		for (j = 0; j < l; j++)
			win_printf(" 0x%02x", p[offset+i+j]);
	}
	win_printf("\n");
}
...
static void dump_ccid_device(const unsigned char *buf)
{
	...
	win_printf("      ChipCard Interface Descriptor:\n"
	       "        bLength             %5u\n"
	       "        bDescriptorType     %5u\n"
	       "        bcdCCID             %2x.%02x",
	       buf[0], buf[1], buf[3], buf[2]);
	...
	win_printf("        dwDefaultClock      %5u\n", us);
	us = convert_le_u32(buf+14);
	win_printf("        dwMaxiumumClock     %5u\n", us);
	win_printf("        bNumClockSupported  %5u\n", buf[18]);
	us = convert_le_u32(buf+19);
	win_printf("        dwDataRate        %7u bps\n", us);
	us = convert_le_u32(buf+23);
	win_printf("        dwMaxDataRate     %7u bps\n", us);
	win_printf("        bNumDataRatesSupp.  %5u\n", buf[27]);

	us = convert_le_u32(buf+28);
	win_printf("        dwMaxIFSD           %5u\n", us);

	us = convert_le_u32(buf+32);
	win_printf("        dwSyncProtocols  %08X ", us);
	if ((us&1))
		fputs(" 2-wire", stdout);
	if ((us&2))
		fputs(" 3-wire", stdout);
	...
	win_printf("        dwMechanical     %08X ", us);
	if ((us & 1))
		fputs(" accept", stdout);
	if ((us & 2))
		fputs(" eject", stdout);
	...
	win_printf("        dwMaxCCIDMsgLen     %5u\n", us);

	win_printf("        bClassGetResponse    ");
	if (buf[48] == 0xff)
		fputs("echo\n", stdout);
	else
		win_printf("  %02X\n", buf[48]);

	win_printf("        bClassEnvelope       ");
	if (buf[49] == 0xff)
		fputs("echo\n", stdout);
	else
		win_printf("  %02X\n", buf[48]);

	win_printf("        wlcdLayout           ");
	if (!buf[50] && !buf[51])
		fputs("none\n", stdout);
	else
		win_printf("%u cols %u lines\n", buf[50], buf[51]);

	win_printf("        bPINSupport         %5u ", buf[52]);
	if ((buf[52] & 1))
	...
	win_printf("        bMaxCCIDBusySlots   %5u\n", buf[53]);
...
static void dump_report_desc(unsigned char *b, int l)
{
	...
	win_printf("          Report Descriptor: (length is %d)\n", l);
	for (i = 0; i < l; ) {
		t = b[i];
		bsize = b[i] & 0x03;
		if (bsize == 3)
		...
		win_printf("            Item(%-6s): %s, data=", types[btype>>2],
				names_reporttag(btag));
		if (bsize > 0) {
			win_printf(" [ ");
			data = 0;
			for (j = 0; j < bsize; j++) {
				win_printf("0x%02x ", b[i+1+j]);
				data += (b[i+1+j] << (8*j));
			}
			win_printf("] %d", data);
		} else
		...
		case 0xa0: /* Collection */
			...
			case 0x04:
				win_printf("Named Array\n");
				break;

			case 0x05:
				win_printf("Usage Switch\n");
				break;

			case 0x06:
				win_printf("Usage Modifier\n");
				break;

			default:
				if (data & 0x80)
					win_printf("Vendor defined\n");
				else
					win_printf("Reserved for future use.\n");
			}
			break;
		case 0x80: /* Input */
		case 0x90: /* Output */
		case 0xb0: /* Feature */
			win_printf("%s%s %s %s %s %s\n%s%s %s %s %s\n",
			       indent,
			    ...
			    const unsigned char *buf)
{
	...
	for (i = 0; i < buf[5]; i++)
		win_printf("          bDescriptorType     %5u %s\n"
	...
	for (i = 0; i < buf[5]; i++) {
		...
		if (len > (unsigned int)sizeof(dbuf)) {
			win_printf("report descriptor too long\n");
			continue;
		}
		if (libusb_claim_interface(dev, interface->bInterfaceNumber) == 0) {
			...
			if (n > 0) {
				if (n < len)
					win_printf("          Warning: incomplete report descriptor\n");
...
dump_comm_descriptor(libusb_device_handle *dev, const unsigned char *buf, char *indent)
{
	...
	case 0x01:		/* call management functional desc */
		...
		if (buf[0] != 5)
			goto bad;
		win_printf("%sCDC Call Management:\n"
		       "%s  bmCapabilities       0x%02x\n",
		       indent,
		       indent, buf[3]);
		if (buf[3] & 0x01)
			win_printf("%s    call management\n", indent);
		if (buf[3] & 0x02)
			win_printf("%s    use DataInterface\n", indent);
		win_printf("%s  bDataInterface          %d\n", indent, buf[4]);
		break;
	case 0x02:		/* acm functional desc */
		...
		win_printf("%sCDC ACM:\n"
		       ...
		       indent, buf[3]);
		if (buf[3] & 0x08)
			win_printf("%s    connection notifications\n", indent);
		if (buf[3] & 0x04)
			win_printf("%s    sends break\n", indent);
		if (buf[3] & 0x02)
			win_printf("%s    line coding and serial state\n", indent);
		if (buf[3] & 0x01)
			win_printf("%s    get/set/clear comm features\n", indent);
		break;
#if 0
	...
	case 0x08:		/* telephone operational modes */
		...
		if (buf[0] != 4)
			goto bad;
		win_printf("%sCDC Telephone operations:\n"
		       "%s  bmCapabilities       0x%02x\n",
		       indent,
		       indent, buf[3]);
		if (buf[3] & 0x04)
			win_printf("%s    computer centric mode\n", indent);
		if (buf[3] & 0x02)
			win_printf("%s    standalone mode\n", indent);
		if (buf[3] & 0x01)
			win_printf("%s    simple mode\n", indent);
		break;
#if 0
	...
	case 0x0a:		/* network channel terminal */
		...
		win_printf("%sNetwork Channel Terminal:\n"
		       "%s  bEntityId               %3d\n"
		       "%s  iName                   %3d %s\n"
		       "%s  bChannelIndex           %3d\n"
	...
	case 0x0f:		/* ethernet functional desc */
		...
		win_printf("%s  wMaxSegmentSize         %10d\n"
	...
	case 0x14:		/* device management functional desc */
		...
		if (buf[0] != 7)
			goto bad;
		win_printf("%sCDC Device Management:\n"
		       "%s  bcdVersion           %x.%02x\n"
		       "%s  wMaxCommand          %d\n",
		       indent,
		       indent, buf[4], buf[3],
	...
	default:
		...
		win_printf("%sUNRECOGNIZED CDC: ", indent);
		dump_bytes(buf, buf[0]);
...
bad:
	win_printf("%sINVALID CDC (%s): ", indent, type);
	dump_bytes(buf, buf[0]);
	...
	for (i = 0; i < buf[2]; i++) {
		...
		if (speed != 0x0300) {
			...
			if (link_state < (sizeof(link_state_descriptions) /
				...
				info_printf("%s", link_state_descriptions[link_state]);
			info_printf("%s%s%s%s\n",
					(status[0] & 0x10) ? " RESET" : "",
					(status[0] & 0x08) ? " oc" : "",
					(status[0] & 0x02) ? " enable" : "",
...
dump_device_status(libusb_device_handle *fd, int otg, int wireless)
{
	...
	info_printf("Device Status:     0x%02x%02x\n",
			status[1], status[0]);
	if (status[0] & (1 << 0))
		info_printf("  Self Powered\n");
	else
		info_printf("  (Bus Powered)\n");
	if (status[0] & (1 << 1))
		info_printf("  Remote Wakeup Enabled\n");
	if (status[0] & (1 << 2)) {
		/* for high speed devices */
		if (!wireless)
			info_printf("  Test Mode\n");
	...
	if (otg) {
		if (status[0] & (1 << 3))
			info_printf("  HNP Enabled\n");
		if (status[0] & (1 << 4))
			info_printf("  HNP Capable\n");
		if (status[0] & (1 << 5))
			info_printf("  ALT port is HNP Capable\n");
	}
	/* for high speed devices with debug descriptors */
	if (status[0] & (1 << 6))
		info_printf("  Debug Mode\n");
	...
	info_printf("Wireless Status:     0x%02x\n", status[0]);
	if (status[0] & (1 << 0))
		info_printf("  TX Drp IE\n");
	if (status[0] & (1 << 1))
		info_printf("  Transmit Packet\n");
	if (status[0] & (1 << 2))
		info_printf("  Count Packets\n");
	if (status[0] & (1 << 3))
		info_printf("  Capture Packet\n");
	...
	info_printf("Channel Info:        0x%02x\n", status[0]);
	...
	info_printf("Transmit Power:\n");
	info_printf("TxNotification:     0x%02x\n", status[0]);
	info_printf("TxBeacon:     :     0x%02x\n", status[1]);
}
...
static void dump_usb2_device_capability_desc(unsigned char *buf)
{
	...
	wide = buf[3] + (buf[4] << 8) +
		(buf[5] << 8) + (buf[6] << 8);
	win_printf("USB 2.0 Extension Device Capability:\n"
			"    bLength             %5u\n"
			"    bDescriptorType     %5u\n"
	...
	if (!(wide & 0x02))
		win_printf("      (Missing must-be-set LPM bit!)\n");
	else
		win_printf("      Link Power Management (LPM)"
				" Supported\n");
}
...
{
	if (buf[0] < 10) {
		win_printf("  Bad SuperSpeed USB Device Capability descriptor.\n");
		return;
	}
	win_printf("    \nSuperSpeed USB Device Capability:\n"
			...
			buf[0], buf[1], buf[2], buf[3]);
	if (!(buf[3] & 0x02))
		win_printf("      Latency Tolerance Messages (LTM)"
				" Supported\n");
	win_printf("    wSpeedsSupported   0x%02x%02x\n", buf[5], buf[4]);
	if (buf[4] & (1 << 0))
		win_printf("      Device can operate at Low Speed (1Mbps)\n");
	if (buf[4] & (1 << 1))
		win_printf("      Device can operate at Full Speed (12Mbps)\n");
	if (buf[4] & (1 << 2))
		win_printf("      Device can operate at High Speed (480Mbps)\n");
	if (buf[4] & (1 << 3))
		win_printf("      Device can operate at SuperSpeed (5Gbps)\n");

	win_printf("    bFunctionalitySupport %3u\n", buf[6]);
	switch(buf[6]) {
	case 0:
		win_printf("      Lowest fully-functional device speed is "
				"Low Speed (1Mbps)\n");
		break;
	case 1:
		win_printf("      Lowest fully-functional device speed is "
	...
	win_printf("    bU1DevExitLat        %4u micro seconds\n", buf[7]);
	win_printf("    bU2DevExitLat    %8u micro seconds\n", buf[8] + (buf[9] << 8));
}

static void dump_container_id_device_capability_desc(unsigned char *buf)
{
	if (buf[0] < 20) {
		win_printf("  Bad Container ID Device Capability descriptor.\n");
		return;
	}
...
void dumpdev(libusb_device *dev)
{
	...
	if (desc.bNumConfigurations) {
		...
		info_printf("\nConfiguration Numbers：%d", desc.bNumConfigurations);
